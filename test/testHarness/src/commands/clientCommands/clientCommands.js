...
const bossPanic = function(inputData, inputMetaData) {
  ...
  if (inputData && inputData.length > 1) {
    ...
    if (inputData.length > 5) {
      fastTypingOutput = haystacks.executeBusinessRule(biz.cstringToBoolean, inputData[5], '');
    }
  ...
  while (true) { // Start the infinite loop
    if (noColoredLineCount <= 0 && enableColoredLine === false) {
      noColoredLineCount = haystacks.executeBusinessRule(biz.crandomlyGenerateNumberInRange, num.c1, [noColoredLinesMaxLength, false, false]);
      enableColoredLine = true;
    }
    if (coloredLineCount <= 0 && enableColoredLine === true) {
      coloredLineCount = haystacks.executeBusinessRule(biz.crandomlyGenerateNumberInRange, num.c2, [coloredLinesMaxLength, false, false]);
      enableColoredLine = false;
    }
    stringLength = haystacks.executeBusinessRule(biz.crandomlyGenerateNumberInRange, num.c1, [lineLength, false, false]);
    ...
    colorBreakPoint = haystacks.executeBusinessRule(biz.crandomlyGenerateNumberInRange, num.c1, [stringLength, false, false]);
    stringToPrint = haystacks.executeBusinessRule(biz.cgenerateRandomMixedCaseAlphaNumericCodeWithSpecialCharactersByLength, stringLength, gen.cMostSpecialCharacters);
    if (enableColoredLine === true && systemColorLogsEnabled === true) {
      ...
      if (haystacks.executeBusinessRule(biz.crandomlyGenerateBooleanValue, '', '') === true) {
        brightColor1 = haystacks.executeBusinessRule(biz.cgenerateRandomBrightColor, 200, 255);
        darkColor1 = haystacks.executeBusinessRule(biz.cgenerateRandomDarkColor, 0, 60);
        colorMode1 = haystacks.executeBusinessRule(biz.crandomlyGenerateBooleanValue, '', '');
        if (colorMode1 === true) {
      ...
      if (haystacks.executeBusinessRule(biz.crandomlyGenerateBooleanValue, '', '') === true) {
        brightColor2 = haystacks.executeBusinessRule(biz.cgenerateRandomBrightColor, 200, 255);
        darkColor2 = haystacks.executeBusinessRule(biz.cgenerateRandomDarkColor, 0, 60);
        colorMode2 = haystacks.executeBusinessRule(biz.crandomlyGenerateBooleanValue, '', '');
        if (colorMode2 === true) {
    ...
    if (fastTypingOutput === true) {
      for (let i = 0; i < stringToPrint.length; i++) {
        ...
        haystacks.sleep(speedTypingPerformanceIndex);
      } // End-for (let i = 0; i < stringToPrint.length; i++)
      console.log('\r'); // Carriage return
    } else {
      console.log(stringToPrint);
    }
    haystacks.sleep(performanceIndex);
